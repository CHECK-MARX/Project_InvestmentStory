...
public sealed class PortfolioAnalyticsServiceTests
{
    ...
    [InlineData("投資信託（金額/旧つみたてNISA預り）", AccountTypes.NisaLegacy)]
    [InlineData("旧NISA", AccountTypes.NisaLegacy)]
    [InlineData("旧ニーサ", AccountTypes.NisaLegacy)]
    [InlineData("特定口座", AccountTypes.Specific)]
    [InlineData("一般口座", AccountTypes.General)]
    [InlineData("NISA(成)", AccountTypes.NisaGrowth)]
    [InlineData("NISA（成）", AccountTypes.NisaGrowth)]
    [InlineData("NISA(つ)", AccountTypes.NisaAccumulation)]
    [InlineData("NISA（つ）", AccountTypes.NisaAccumulation)]
